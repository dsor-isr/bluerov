    ...
    def timerIterCallback(self, event=None):
        ...
        if all(h_ is not None for h_ in self.h): 
            ...
            if self.body_velocity is not None and self.attitude is not None:
                ...
                self.D_pub.publish(Vector3(self.bottom_follower.xhat[0], self.bottom_follower.xhat[1], self.bottom_follower.xhat[2]))
            ...
            self.debug_msg.distance = np.linalg.norm(self.bottom_follower.xhat)
            self.debug_msg.terrain_normal_body_frame.x = self.bottom_follower.n[0]
            self.debug_msg.terrain_normal_body_frame.y = self.bottom_follower.n[1]
            self.debug_msg.terrain_normal_body_frame.z = self.bottom_follower.n[2]
